...
import { ShareSheet, type ShareOption } from '../../components/insights/ShareSheet';
...
export default function UnderstandScreen() {
  ...
  // Phase 15.11 — single Share CTA opens this sheet. Default closed.
  ...
  const handleShareSelection = async (option: ShareOption) => {
    ...
    // Explicit per-key title mapping rather than an else fallback —
    // a future fourth ShareOption added without a case here will
    // surface as a type error.
    let title: string;
    if (option === 'care-report') {
      title = 'Care report';
    } else if (option === 'medication-report') {
    ...
    try {
      ...
      await Share.share({ message: `${title} — Generated by EmberMate` });
            ...
            return (
              <View style={styles.section}>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                  accessibilityRole="button"
                  accessibilityLabel={`${buttonLabel}. Opens a menu of share options.`}
                >
