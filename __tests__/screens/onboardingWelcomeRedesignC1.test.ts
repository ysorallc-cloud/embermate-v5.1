...
describe('Onboarding redesign C1 — Welcome screen', () => {
  ...
  it('contract 4 (CHARCOAL INK CTA GRADIENT): the shared ONBOARDING_CTA_GRADIENT bridge palette is imported; no slab-green (#5fb88a) in source', () => {
    ...
    // ['#e09556', '#c98a4a'] exported from constants/onboardingTokens.ts
    // (moved out of app/ — a constants file under the routes tree
    // triggered an expo-router "missing default export" route warning).
    // The website's colors.ember / colors.emberDeep theme tokens stay
    // untouched for other in-app surfaces; onboarding routes through the
    // dedicated constant so the palette can drift independently.
    expect(SRC).toMatch(/ONBOARDING_CTA_GRADIENT/);
    expect(SRC).toMatch(/from\s+['"]\.\.\/\.\.\/\.\.\/constants\/onboardingTokens['"]/);
