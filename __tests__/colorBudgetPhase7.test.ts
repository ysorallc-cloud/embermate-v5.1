...
// DECORATIVE accents:
//
//   • #9ccfa6  sage              (primary CTAs, completed states, Now)
//   • #aa8adc  caregiverAccent   (lavender — caregiver-facing surfaces)
//   • #e3a684  criticalAlert     (genuine emergencies / destructive)
//
...
// color-name family (#e3a684 — the actual coral hue, same as
// criticalAlert) to claim the `coral*` namespace. The 3-budget intent
// survives — `coral` and `criticalAlert` now both resolve to #e3a684
...
describe('Phase 7 — 3-accent budget enforced', () => {
  it('the 3 budget accents are defined in theme-tokens', () => {
    ...
    expect(src).toMatch(/accent:\s*['"]#9ccfa6['"]/);
    // F7 (2026-06-12) — caregiverAccent flipped from #aa8adc (warm
    // lavender) to #8fa8c8 (dusty blue). Token name preserved.
    expect(src).toMatch(/caregiverAccent:\s*['"]#8fa8c8['"]/);
    expect(src).toMatch(/criticalAlert:\s*['"]#e3a684['"]/);
  });
